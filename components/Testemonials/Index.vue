...
export default {
  ...
  methods: {
    ...
    stepPrev() {
      ...
      if (this.active === 1) {
        // clone items backwards
        console.log('clone items backwards')

        this.isClone = true
        this.active = this.initialListLenght

        let wrapperMl = `${500 * -1 * (this.active + 1)}px`

        this.$refs.itemsWrapper.style.marginLeft = wrapperMl

        console.log(
          'recuado ' +
            (this.active + 1) +
            ' para tras:' +
            `${500 * -1 * (this.active + 1)}px`
        )

        setTimeout(() => {
          const prevItem = this.active - 1
          this.$refs.text[prevItem].style.transitionProperty = 'margin-left'
          this.$refs.client[prevItem].style.transitionProperty = 'margin-left'
          this.$refs.text[prevItem].style.marginLeft = 0
          this.$refs.client[prevItem].style.marginLeft = 0

          console.log('text e client ml 0 no prevItem')
        }, 3000)

        setTimeout(() => {
          wrapperMl = Number(wrapperMl.replace('px', '')) + 500
          this.$refs.itemsWrapper.style.marginLeft = `${wrapperMl}px`
          console.log('margin left no itens wrapper - added 500px')
        }, 6000)

        setTimeout(() => {
          this.$refs.text[this.active - 1].style.marginLeft = `${Number(
            this.$refs.text[this.active - 1].style.marginLeft.replace('px', '')
          ) + 500}px`

          this.$refs.client[this.active - 1].style.marginLeft = `${Number(
            this.$refs.client[this.active - 1].style.marginLeft.replace(
              'px',
              ''
            )
          ) + 500}px`

          console.log('added 500px no texto e cliente do ' + (this.active - 1))

          // try {
          //   this.$refs.text[this.active + 2].style.marginLeft = '500px'
          //   this.$refs.client[this.active + 2].style.marginLeft = '500px'
          // } catch (e) {}
        ...
        // this.$refs.text[this.initialListLenght].style.transitionProperty =
        //   'margin-left'
        // this.$refs.client[this.initialListLenght].style.transitionProperty =
        //   'margin-left'
        ...
        // this.$refs.text[active - 1].style.marginLeft = `${Number(
        //   this.$refs.text[active - 1].style.marginLeft.replace('px', '')
        // ) + width}px`

        // this.$refs.client[active - 1].style.marginLeft = `${Number(
        //   this.$refs.client[active - 1].style.marginLeft.replace('px', '')
        // ) + width}px`

        // this.$refs.text[active].style.marginLeft = `${Number(
        //   this.$refs.text[active].style.marginLeft.replace('px', '')
        // ) +
        //   width * 2}px`

        // this.$refs.client[active].style.marginLeft = `${Number(
        //   this.$refs.client[active].style.marginLeft.replace('px', '')
        // ) +
        //   width * 2}px`
      } else {
        ...
        try {
          const c = this.isClone

          this.$refs.text[active - 1].style.transitionProperty = 'margin-left'
          this.$refs.client[active - 1].style.transitionProperty = 'margin-left'
          this.$refs.text[active - 1].style.marginLeft = c ? `${width}px` : 0
          this.$refs.client[active - 1].style.marginLeft = c ? `${width}px` : 0

          if (c) {
            this.$refs.itemsWrapper.style.marginLeft = `${Number(
              this.$refs.itemsWrapper.style.marginLeft.replace('px', '')
            ) + width}px`
          }
        } catch (e) {}
